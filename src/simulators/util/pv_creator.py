...
class PVGenerator:
    ...
    def GenerateCSV(PVGen, OUTPUT_DIR = OUTPUT_DIR):
        ...
        logger.info("Consolidating curves into CSV files (otimizado)...")
        ...
        logger.info(f"Success! Optimized CSV files saved to {OUTPUT_DIR}")
    ...
    def GenerateDSS(PVGen, OUTPUT_DIR=OUTPUT_DIR):
        ...
        for i, pv in enumerate(PVGen):
            ...
            # Extract multipliers for LoadShapes (only if not created yet)
        ...
        # 3. Add necessary Redirects and base curves at the top of the file
        ...
        # 4. Write all configurations to their respective DSS files
        ...
        logger.info(f"Success! Optimized '{OUTPUT_DSS_FILE.name}' saved to {OUTPUT_DIR}")
